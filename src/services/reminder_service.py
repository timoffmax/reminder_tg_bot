...
class ReminderService:
    ...
    # Message prefix generated by create_lead_time_reminder; identifies lead-time children.
    LEAD_TIME_PREFIX_PATTERN = re.compile(r'^⏳ (\d+)m before: ')
    ...
    def change_repeat_schedule(
        ...
        user_tz = self._get_user_tz(reminder.user_id)
        new_time = self._align_to_pattern(
            anchor_time if anchor_time is not None else reminder.scheduled_time,
            repeat_interval,
            user_tz,
        )

        # Never schedule the first occurrence past a user-defined end date;
        # the end date must be cleared first.
        if (reminder.repeat_until is not None) and (new_time > reminder.repeat_until):
            return None
        ...
        self._cascade_schedule_to_lead_time_children(reminder)
    ...
    def _cascade_schedule_to_lead_time_children(self, parent: Reminder) -> None:
        """Keep lead-time alerts in lockstep with the parent's new schedule (best-effort)."""
        children = self.db.query(Reminder).filter(
            Reminder.parent_reminder_id == parent.id,
            Reminder.status.in_([
                ReminderStatus.ACTIVE.value,
        ...
        for child in children:
            match = self.LEAD_TIME_PREFIX_PATTERN.match(child.message_text or '')

            if match is None:
                continue

            lead_minutes = int(match.group(1))
            child.reminder_type = parent.reminder_type
            child.repeat_interval = parent.repeat_interval
            child.repeat_until = parent.repeat_until
            child.scheduled_time = parent.scheduled_time - timedelta(minutes=lead_minutes)

    def get_pending_child_ids(self, reminder_id: int) -> List[int]:
        """IDs of not-yet-finished children (follow-ups, lead-time alerts) of a reminder."""
        children = self.db.query(Reminder).filter(
            Reminder.parent_reminder_id == reminder_id,
            Reminder.status.in_([
                ReminderStatus.ACTIVE.value,
    ...
    def _align_to_pattern(
        ...
        # Terminates: every advance moves at least one day forward, and the
        # weekday constraint only applies with the one-day step (multi-day).
        while True:
